...
const Trivia =(props)=>{
    ...
    const fetchData = async ()=>{
        const trivia = await axios.get(`http://numbersapi.com/${inputData == null ? "random" : inputData}/trivia?json`)
        setData(trivia.data)
        setLoading(false)
    }
    useEffect(()=>{
    ...
    return(
        <KeyboardAvoidingView style={{flex : 1}}>
            <SafeAreaView style={styles.container}>
                    ...
                    <Input placeholder ={"Enter a number..."} input ={(text)=>setInputData(text)} submit ={()=>{
                        fetchData()
                        setInputData(null)
                        }} />
