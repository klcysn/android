...
const Year =(props)=>{
    ...
    const fetchData = async ()=>{
        const trivia = await axios.get(`http://numbersapi.com/${inputData == null ? "random" : inputData}/year?json`)
        setData(trivia.data)
        setLoading(false)
    }
    useEffect(()=>{
    ...
    return(
        <KeyboardAvoidingView style={{flex : 1}}>
            <SafeAreaView style={styles.container}>
                    ...
                    <Input placeholder ={"Enter a year..."} input ={(text)=>setInputData(text)} submit ={()=>{
                        fetchData()
                        setInputData(null)
                        }} />
