...
import {TouchableOpacity, Image, Text, SafeAreaView, FlatList} from "react-native"
...
const PostCard =(props)=>{
    function renderPost({item}){
        return(
            <TouchableOpacity style={{margin : 5}} onPress={()=>props.navigation.navigate("Person",{data:item})}>
                <Image source = {{uri:item.img}} style ={styles.image} />
                <Text style = {styles.timeLineHeader}>{item.userName}</Text>
